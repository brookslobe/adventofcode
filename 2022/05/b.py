...
while instructions_idx_data <= last_line_num:
    ...
    for m in range(0, moves_todo): # stop arg in range is exclusive
        ...
        multi_char_grab.append(c) # "top" of aisle to bottom

    for c in range(0, len(multi_char_grab)):
        c = multi_char_grab.pop() # pulling from top of temp stack (previously, bottom of aisle) to retain order
